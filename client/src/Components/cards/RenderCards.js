  ...
  const fetchCardsData = async () => {
    try {
      const response = await fetch(
        `http://localhost:4040/Ebartr/cards/cardsdata/${
    ...
    <div>
      <nav className="bg-gray-800 px-4 py-2 flex lg:flex-row items-center flex-shrink-0">
        <div className="flex justify-between items-center">
          <span className="text-white text-xl ml-6 lg:mb-0 sm:mb-2">
            EBARTR
          </span>
        </div>
        <div className="flex flex-grow justify-center" id="navbar-collapse">
          <form onSubmit={handleSubmit} className="w-2/3 flex items-center">
            ...
            <button
              type="submit"
              className="bg-white h-10 w-10 flex justify-center items-center ml-0 rounded-r-lg cursor-pointer border"
            >
        ...
        <div className="flex my-3 my-0">
          ...
          <a href={isLoggedIn !== null ? "sell" : "loginsignup"}>
          ...
          <a href="LoginSignup">
            <button
              className="bg-purple-700 hover:bg-purple-800 text-white py-1 px-3 rounded border border-solid border-purple-700 hover:border-purple-800 hidden lg:block"
              onClick={handleOp}
            >
      ...
      <div className="card-list">
        {cards.map((card) => (
          <div
            key={card._id}
            onClick={() =>
              handleCardClick(card._id, card.position, card.userID)
            }
          >
